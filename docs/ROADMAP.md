...
**进展(2026-07-12,分支)——多次采样取一致已实现**:
- `--samples N`(1~3,check/review/browser 均支持):每轮在 prompt 注入轮次标记(区分缓存键 + 驱动样本多样性),独立生成/执行/修复。
- 合并规则:**检出并集**——任一轮断言失败即判失败并保留该轮证据,已失败判定不被后续轮"洗白"(真机观察到的波动都是漏检方向,并集把"看运气 3~4/5"收敛为稳定检出上界)。
- 真机 e2e:弱断言轮通过 + 强断言轮失败 → 合并判失败,附强断言轮证据。

**✅ 验收通过(2026-07-12),1.8 完成合入 main**:DeepSeek 真机 6 轮独立复测(单采样 ×3 + `--samples 2` ×3)全部 ≥4/5、多数 **5/5**(历史 3~4/5、最差 3/5);并集救援直接可见(某轮首轮 3/5 被拉回 4/5);历史从未检出的 DEMO-005(localStorage)6/6 稳定检出;断言强度护栏在真实模型下正确拒绝弱化"修复"。验收中暴露并修复:断言目标为非交互元素时 LLM 猜类名误报 → DOM 摘要补充**地标元素**(带 id 的非交互节点),复测误报消除。详见 [ACCEPTANCE.md](ACCEPTANCE.md) 1.8 验收节。
