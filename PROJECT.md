...
用户选择走 Phase 2（易用性)。**已完成 2.6 HTML 报告**（插队优先,`review --html`)与 **2.1 一键 `check` 命令**（主入口,scan→精解析→编排执行→浏览器→诊断→HTML,启动命令交互确认安全边界)。**2.2 配置文件 `specprobe.toml`**（`init` 生成模板,优先级 CLI > 环境 > 配置 > 默认,check 已接入)。Phase 2 剩余:2.3 进度条、2.4 运行归档/SQLite、2.5 审批持久化、2.7 跨平台。
